...
def getRobotModelFloating(robot_name="hyq"):
    ...
    path = rospkg.RosPack().get_path('giraffe_description')
    if rosgraph.is_master_online():
        try:
            urdf = ros.get_param('/robot_description')
            print("URDF generated_commons")
            os.makedirs(path + "/urdf/generated_urdf/", exist_ok=True)
        ...
        try:
            ...
            print('you are running offline, urdf is not present in urdf/generated_urdf folder')
...
def getRobotModel(robot_name="hyq", generate_urdf=False, xacro_path=None, additional_urdf_args=None):
    ...
    if (generate_urdf):
        try:
            ...
            args = xacro_path + ' --inorder -o ' + rospkg.RosPack().get_path('giraffe_description') + '/urdf/generated_urdf/' + robot_name + '.urdf'
...
def getRobotModel(robot_name="hyq", generate_urdf = True, xacro_path = None, additional_urdf_args = None, floating_base=False):
    ERROR_MSG = 'You should set the environment variable LOCOSIM_DIR"\n';
    path  = path = rospkg.RosPack().get_path('giraffe_description')
    srdf  = path + "/urdf/" + robot_name + ".srdf"
    ...
    if (generate_urdf):  
        try:       
            #old way
            if (xacro_path is None):
                xacro_path = rospkg.RosPack().get_path('giraffe_description')+ '/urdf/'+robot_name+'.urdf.xacro'
            ...
            # with gazebo 11 you should set in the ros_impedance_controllerXX.launch the new_gazebo_version = true
            # note we generate the urdf with the floating base joint (new gazebo version should be false by default in the xacro of the robot! because Pinocchio needs it!
            urdf_dir = os.path.join(path, 'urdf', 'generated_urdf')
            os.makedirs(urdf_dir, exist_ok=True)
            urdf_output_path = os.path.join(urdf_dir, robot_name + '.urdf')
            args = xacro_path + ' --inorder -o ' + urdf_output_path
            ...
            os.system("xacro " + args)  
            #os.system("rosparam get /robot_description > "+os.environ['LOCOSIM_DIR']+'/urdf/'+robot_name+'.urdf')  
            #urdf = URDF.from_parameter_server()
            print("URDF generated_commons")
            urdf_location      = path + "/urdf/generated_urdf/" + robot_name+ ".urdf"
            print(urdf_location)
