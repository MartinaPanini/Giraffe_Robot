...
def postural_target_computation(pitch_des_final):
    eps = 1e-4
    IT_MAX = 5000
    damp = 1e-5
...
def generate_trajectory(t, pitch_des_final):
    p0, _, pitch0 = FK()
    # Posizione
...
def simulation(pitch_des_final, q, qd):
    time = 0.0
    log_counter = 0
    buffer_size, q_log, p_log, p_des_log, pitch_log, pitch_des_log, time_log = log_initialize()
    q0_calibrated = postural_target_computation(pitch_des_final)
    ...
    while time < conf.exp_duration:
        ...
        p_des, v_des, a_des, pitch_des, pitch_vel_des, pitch_acc_des = generate_trajectory(time, pitch_des_final)
